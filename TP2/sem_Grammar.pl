...
vp(FV,PVP,TVP) --> v(FV,PV,TV), pp(FPP,PPP,TPP), pp(FPP2,PPP2,TPP2), { % two ditransitive verb, eg. 'talk'
		att(FPP, gloss, FPP_name),
		att(FPP2,gloss, FPP2_name),
		att(FV, cpl, [pp(FPP_name),pp(FPP2_name)]),
		link(2, PV,PPP,PVP),	% performs some variable unification
		link(3, PV,PPP2,PVP),	% performs some (other) variable unification
		TVP  = vp(TV, TPP, TPP2) }.
...
n([gloss:daughter,  num:plur], daughter(_,_),  n(daughter))  --> [daughters].
